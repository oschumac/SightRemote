...
import zbp.rupbe.sightparser.applayer.descriptors.DescriptorEnum;
...
public class ByteBuf {
    ...
    public void putEnum(DescriptorEnum descriptorEnum) {
        putShort(descriptorEnum.getValue());
    }

    public DescriptorEnum getEnum(Class<? extends Enum> clazz, int position) {
        try {
            short value = getShort(position);
            Method method = clazz.getMethod("getValue");
            for (Enum e : clazz.getEnumConstants()) if ((short) method.invoke(e) == value) return (DescriptorEnum) e;
    ...
    public DescriptorEnum getEnum(Class<? extends Enum> clazz) {
        return getEnum(clazz, 0);
    }

    public DescriptorEnum readEnum(Class<? extends Enum> clazz) {
        DescriptorEnum descriptorEnum = getEnum(clazz);
        shift(2);
        return descriptorEnum;
    }
}
